...
def calculateMaximumFPS(url, length=5.0):
  imageGrabber = ImageGrabber(url)
  timer = SimpleTimer()
  count = 0
  while timer.Elapsed() <= length:
    imageGrabber.getFrame()
    count += 1
  time = timer.Elapsed()
  return count / time
...
def main(script_name, url=None):
  ...
  # logging.info('Maximum FPS detected is {}'.format(calculateMaximumFPS(url)))
  ...
  def cleanUp():
    ...
    asyncImageGrabber.clean()

  while True:
    if not asyncImageGrabber.running:
      cleanUp()
      return 1
    image = asyncImageGrabber.get()
    # asyncImageGrabber.queue.task_done()
    # try:
    #   image = imageGrabber.getFrame()
    # except Exception as e:
    #   logging.error('Failed to grab image due to exception(%s)' % (str(e),))
